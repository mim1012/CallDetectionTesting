...
class KakaoTaxiAccessibilityService : AccessibilityService() {
    ...
    /**
     * MediaProjection 팝업 자동 처리
     */
    private fun handleMediaProjectionPopup() {
        serviceScope.launch {
            try {
                Log.d(TAG, "🤖 MediaProjection 팝업 자동 처리 시도...")
                ...
                if (rootNode != null) {
                    ...
                    if (!buttonFound) {
                        Log.w(TAG, "⚠️ 팝업 버튼을 찾을 수 없음. 수동 클릭 필요")
                        showToast("수동으로 '지금 시작' 클릭하세요!")
                    }
                    
                    rootNode.recycle()
                } else {
                    Log.w(TAG, "⚠️ 접근성으로 화면 정보를 가져올 수 없음")
                }
                
            } catch (e: Exception) {
                Log.e(TAG, "❌ 팝업 자동 처리 실패", e)
            }
        }
    }
